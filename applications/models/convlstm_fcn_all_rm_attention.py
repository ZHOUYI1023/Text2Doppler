...
class SymmetricLightweightConv(nn.Module):
    def __init__(self, d_size, groups=2, kernel_size=3, padding=1, n_heads=1,
        ...
        self.weight = nn.Parameter(torch.Tensor(int(n_heads*self.input_size/2), 1, kernel_size))
    ...
    def forward(self, input):
        ...
        #print(weight.shape)
        output = F.conv1d(reshaped_tensor, weight, padding=self.padding, groups=int(self.groups/2))
        #print(output.shape)
